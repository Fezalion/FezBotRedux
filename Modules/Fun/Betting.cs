...
namespace FezBotRedux.Modules.Fun
{
    ...
    public class Betting : InteractiveBase<SocketCommandContext>
    {
        ...
        public class SubModuleBet : InteractiveBase<SocketCommandContext>
        {
            ...
            public async Task GetCash([Summary("User Mention")] IUser userid)
            {
                using (var db = new NeoContext())
                {
            ...
            public async Task GetCash()
            {
                using (var db = new NeoContext())
                {
                    if (db.Users.Any(nbu => nbu.Id == Context.User.Id))
                    {
                        var cash = db.Users.FirstOrDefault(u => u.Id == Context.User.Id);
                        var embed = NeoEmbeds.Minimal($"You have {cash.Cash}₺").Build();
                        await ReplyAsync("", false, embed);
                    }
                    else
                    {
                        var embed = NeoEmbeds.Minimal($"You are not registered.").Build();
                        await ReplyAsync("", false, embed);
                    }
                }
            }
